...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        # print("Input x shape:", x.shape)  # 打印输入维度
        h = self.heads
        ...
        # print("Output shape after CrossAttention:", out.shape)  # 打印输出维度
...
class FactorAtt_ConvRelPosEnc(nn.Module):
    ...
    def __init__(
        ...
        self.k_conv = nn.Conv1d(35, 196, kernel_size=1)
        self.v_conv = nn.Conv1d(35, 196, kernel_size=1)
    ...
    def forward(self, q, k, v, minus=True):
        ...
        # 如果 k 和 v 的维度不匹配，使用不同的投影来对齐

        k = self.k_conv(k)  # 投影 k
        v = self.v_conv(v)  # 投影 v
...
class IQARegression(nn.Module):
    def __init__(self, inchannels=768, outchannels=512, reduction=4):
        ...
        # self.cross_attention = MHCABlock(outchannels)
...
class Train:
    ...
    def create_model(self):
        ...
        self.regressor = IQARegression().to(device)
         ...
         # 从本地加载模型
        local_model_path = os.path.expanduser("~/.cache/ImageReward/ImageReward.pt")
        if os.path.exists(local_model_path):
            self.reward = ImageReward.load(local_model_path).to(device)
        else:
    ...
    def train_epoch(self, epoch):
        ...
        for data in tqdm(self.train_loader):
            ...
            image_prompt = data[0]
            text_prompt = data[3]
            text_input = self.reward.blip.tokenizer(text_prompt, padding='max_length', truncation=True, max_length=35, return_tensors="pt",local_files_only=True,cache_dir="/mydata/shared/data_insupur/luwen/yekeyuan_luwen/MoE-AGIQA-master/bert-base-uncased").to(device)
...
def parse_args():
    ...
    parser.add_argument('--checkpoints_dir', type=str, default='/mydata/shared/data_insupur/luwen/yekeyuan_luwen/MoE-AGIQA-master/MoE-AGIQA-master/checkpoint/', \
    ...
    if args.database == 'AGIQA-3K':
        args.data_info = '/mydata/shared/data_insupur/luwen/yekeyuan_luwen/MoE-AGIQA-master/MoE-AGIQA-master/data/AGIQA-3K.mat'
        args.im_dir = '/mydata/shared/data_insupur/luwen/yekeyuan_luwen/dataset/AGIQA-3K/512x512/'
